...
public class CharReader
	...
	public void readCharThrow(CharReaderParams prms,
		...
		if(ti.index < prms.streamStartIndex.index) {
			dat.state = CharReaderData.STATE_END_OF_STREAM;
			return;
		}
		...
		if(bi.versionNumber != readBuf.versionNumber
			...
			getBufferIndexThrow(prms, ti, bi);
		...
		while(true) {
			...
			if(biTemp.versionNumber != readBuf.versionNumber
				...
				getBufferIndexThrow(prms, tiTemp, biTemp);
			...
			if(unitIndex == 0) {
				...
				if(unitCount == 0) {
					...
					dat.state = CharReaderData.STATE_HAVE_CHAR;
				...
				if((unitCount == 1) || (unitCount == 7)) {
					...
					dat.state = CharReaderData.STATE_HAVE_BAD_CHAR;
					return;
				}
	...
	public void readChar(CharReaderParams prms,
		TextIndex ti, BufferIndex bi) {
	...
	public void readJavaStringThrow(CharReaderParams prms,
		TextIndex ti, long len) {
		...
		while(iDone < len) {
			readCharThrow(prms, ti2, bi2);
			state = dat.state;
			
			if(state == CharReaderData.STATE_HAVE_CHAR) {
				charLength = dat.resultCharLength;

				goodCharCount += 1;

				// add char
				sb.appendCodePoint(dat.resultChar);
			...
			if(state == CharReaderData.STATE_END_OF_STREAM)
				throw makeIndexOutOfBoundsException(null);
			
			if(state == CharReaderData.STATE_HAVE_BAD_CHAR) {
				badCharCount += 1;
		...
		dat.state = CharReaderData.STATE_HAVE_JAVA_STRING_BUFFER;
	...
	public void readJavaString(CharReaderParams prms,
		TextIndex ti, long len) {
		...
		try {
			readJavaStringThrow(prms, ti, len);
		} catch(Throwable e2) {
			ex = e2;
		}
	...
	public void readUtf32Throw(CharReaderParams prms,
		TextIndex ti, long len) {
		...
		dat.state = CharReaderData.STATE_HAVE_BUFFER_STRING;
		dat.resultBadCharCount = badCharCount;
		dat.resultGoodCharCount = goodCharCount;
	...
	public void readUtf32(CharReaderParams prms,
		TextIndex ti, long len) {
	...
	public void getBufferIndexThrow(CharReaderParams prms,
		TextIndex ti, BufferIndex bi) {
	...
	public void getBufferIndex(CharReaderParams prms,
		TextIndex ti, BufferIndex bi) {
