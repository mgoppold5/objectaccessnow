...
public class ExtendedFloatTokenHelper 
	implements TokenHelper {
	
	public GeneralUtils utils;
	public ExtendedFloatTokenHelperData dat;
	...
	public void advanceOneChar(CharReaderContext cc) {
		...
		if(dat.state == BaseModuleData.STATE_START) {
			...
			if(c == '0') {
				dat.miniState =
					ExtendedFloatTokenHelperData.STATE_INITIAL_ZERO;
		...
		if(!dat.unsure) {
			...
			if(state == ExtendedFloatTokenHelperData.STATE_INTEGER) {
				...
				if((dat.radix == 2) && utils.isBinaryChar(c)) {
					dat.resultLength += len;
					dat.currentIndex += len;
					return;
				}
				...
				if(c == '_') {
					dat.miniState =
						ExtendedFloatTokenHelperData.STATE_INTEGER_UNDERSCORE;
					// dat.resultLength += len;
					dat.unsureLength = len;
					dat.currentIndex += len;
					dat.unsure = true;
				...
				if(c == '.') {
					dat.miniState =
						ExtendedFloatTokenHelperData.STATE_RADIX_POINT;
					utils.copyTextIndex(dat.integerPastIndex, ti);
					utils.copyTextIndex(dat.pastIndex, ti);
					// dat.resultLength += len;
					dat.unsureLength = len;
				...
				if(isExponentMark(c)) {
					dat.miniState =
						ExtendedFloatTokenHelperData.STATE_EXPONENT_MARK;
					utils.copyTextIndex(dat.integerPastIndex, ti);
					utils.copyTextIndex(dat.pastIndex, ti);
					// report.resultLength += len;
					dat.unsureLength = len;
			...
			if(state == ExtendedFloatTokenHelperData.STATE_FRACTION) {
				if((dat.radix == 16) && utils.isHexChar(c)) {
					dat.resultLength += len;
					dat.currentIndex += len;
					return;
				...
				if(c == '_') {
					dat.miniState =
						ExtendedFloatTokenHelperData.STATE_FRACTION_UNDERSCORE;
					// dat.resultLength += len;
					dat.unsureLength = len;
					dat.currentIndex += len;
					dat.unsure = true;
				...
				if(isFlagSingle(c)) {
					dat.flagSize = 32;
					dat.miniState = ExtendedFloatTokenHelperData.STATE_FLAG;
					utils.copyTextIndex(dat.fractionPastIndex, ti);
					dat.resultLength += len;
					dat.currentIndex += len;
					return;
			...
			if(state == ExtendedFloatTokenHelperData.STATE_EXPONENT) {
				if((dat.radix == 16) && utils.isHexChar(c)) {
					dat.resultLength += len;
					dat.currentIndex += len;
					return;
				...
				if(c == '_') {
					dat.miniState =
						ExtendedFloatTokenHelperData.STATE_EXPONENT_UNDERSCORE;
					// report.resultLength += len;
					dat.unsureLength = len;
					dat.currentIndex += len;
					dat.unsure = true;
			...
			if(state == ExtendedFloatTokenHelperData.STATE_FLAG) {
				dat.state = TokenHelperData.STATE_DONE;
				utils.copyTextIndex(dat.pastIndex, ti);
				return;
			}
		...
		if(dat.unsure) {
			...
			if(state == ExtendedFloatTokenHelperData.STATE_INTEGER_UNDERSCORE) {
				if(c == '_') {
					// dat.resultLength += len;
					dat.unsureLength += len;
					dat.currentIndex += len;
				...
				if((dat.radix == 2) && utils.isBinaryChar(c)) {
					dat.miniState = ExtendedFloatTokenHelperData.STATE_INTEGER;
			...
			if(state == ExtendedFloatTokenHelperData.STATE_FRACTION_UNDERSCORE) {
				if(c == '_') {
					// report.resultLength += len;
					dat.unsureLength += len;
					dat.currentIndex += len;
				...
				if((dat.radix == 2) && utils.isBinaryChar(c)) {
					dat.miniState = ExtendedFloatTokenHelperData.STATE_FRACTION;
					dat.resultLength += len + dat.unsureLength;
					dat.currentIndex += len;
					dat.unsure = false;
					return;
			...
			if(state == ExtendedFloatTokenHelperData.STATE_EXPONENT_UNDERSCORE) {
				if(c == '_') {
					// dat.resultLength += len;
					dat.unsureLength += len;
					dat.currentIndex += len;
				...
				if((dat.radix == 2) && utils.isBinaryChar(c)) {
					dat.miniState = ExtendedFloatTokenHelperData.STATE_EXPONENT;
					dat.resultLength += len + dat.unsureLength;
					dat.currentIndex += len;
					dat.unsure = false;
					return;
			...
			if(state == ExtendedFloatTokenHelperData.STATE_INITIAL_ZERO) {
				if(c == 'h') {
					dat.miniState =
						ExtendedFloatTokenHelperData.STATE_NUMBER_START;
					dat.radix = 16;
					dat.unsureLength += len;
					dat.currentIndex += len;
					return;
			...
			if(state == ExtendedFloatTokenHelperData.STATE_NUMBER_START) {
				if((dat.radix == 16) && utils.isHexChar(c)) {
			...
			if(state == ExtendedFloatTokenHelperData.STATE_RADIX_POINT) {
				if((dat.radix == 16) && utils.isHexChar(c)) {
			...
			if(state == ExtendedFloatTokenHelperData.STATE_EXPONENT_MARK) {
				if((dat.radix == 16) && utils.isHexChar(c)) {
				...
				if(c == '+') {
					dat.miniState =
						ExtendedFloatTokenHelperData.STATE_EXPONENT_SIGN;
					dat.unsureLength += len;
					dat.currentIndex += len;
					return;
				}
			...
			if(state == ExtendedFloatTokenHelperData.STATE_EXPONENT_SIGN) {
				if((dat.radix == 16) && utils.isHexChar(c)) {
					utils.copyTextIndex(dat.exponentStartIndex, ti);
					dat.miniState = ExtendedFloatTokenHelperData.STATE_EXPONENT;
					dat.hasExponent = true;
