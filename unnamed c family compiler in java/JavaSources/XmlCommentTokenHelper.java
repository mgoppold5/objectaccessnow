...
public class XmlCommentTokenHelper
	implements TokenHelper {
	
	public XmlCommentTokenHelperData dat;
	...
	public void advanceOneChar(CharReaderContext cc) {
		...
		if(state == XmlCommentTokenHelperData.STATE_BEGIN_STRING) {
		...
		if(state == XmlCommentTokenHelperData.STATE_MIDDLE_SPAN) {
			if(c == '-') {
				utils.copyTextIndex(dat.contentPastIndex, ti);
				
				dat.miniState = XmlCommentTokenHelperData.STATE_END_HYPHEN1;
				dat.currentIndex += len;
				dat.resultLength += len;
				return;
			}
		...
		if(state == XmlCommentTokenHelperData.STATE_END_HYPHEN1) {
			if(c == '-') {
				dat.miniState = 
					XmlCommentTokenHelperData.STATE_END_HYPHEN2;
				dat.currentIndex += len;
				dat.resultLength += len;
				return;
			}
			...
			dat.miniState =
				XmlCommentTokenHelperData.STATE_MIDDLE_SPAN;
			dat.currentIndex += len;
			dat.resultLength += len;
			return;
		}

		if(state == XmlCommentTokenHelperData.STATE_END_HYPHEN2) {
			if(c == '>') {
				dat.miniState = 
					XmlCommentTokenHelperData.STATE_END_GREATER_THAN;
				dat.currentIndex += len;
				dat.resultLength += len;
				return;
			}
		...
		if(state == XmlCommentTokenHelperData.STATE_END_GREATER_THAN) {
			utils.copyTextIndex(dat.pastIndex, ti);
	...
	public void handleBeginStringMatchStopped(
		...
		if(c == '-') {
			...
			dat.miniState =
				XmlCommentTokenHelperData.STATE_END_HYPHEN1;
			dat.currentIndex += len;
			dat.resultLength += len;
			return;
		}
	...
	public void processEndOfStream(CharReaderContext cc) {
		...
		if(state == XmlCommentTokenHelperData.STATE_MIDDLE_SPAN
			|| state == XmlCommentTokenHelperData.STATE_END_HYPHEN1
			|| state == XmlCommentTokenHelperData.STATE_END_HYPHEN2) {
	...
	private void unterminated(TextIndex ti) {
		...
		e2 = new LangError();
		e2.id = LangErrors.ERROR_XML_COMMENT_UNTERMINATED;
		e2.context = context;
