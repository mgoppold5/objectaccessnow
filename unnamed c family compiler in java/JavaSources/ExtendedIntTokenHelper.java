...
public class ExtendedIntTokenHelper 
	implements TokenHelper {
	
	public GeneralUtils utils;
	public ExtendedIntTokenHelperData dat;
	...
	public void advanceOneChar(CharReaderContext cc) {
		...
		if(dat.state == BaseModuleData.STATE_START) {
			...
			if(c == '0') {
				dat.miniState = ExtendedIntTokenHelperData.STATE_INITIAL_ZERO;
		...
		if(!dat.unsure) {
			...
			if(state == ExtendedIntTokenHelperData.STATE_INTEGER) {
				...
				if(c == '_') {
					dat.miniState =
						ExtendedIntTokenHelperData.STATE_INTEGER_UNDERSCORE;
					// dat.resultLength += len;
					dat.unsureLength = len;
					dat.currentIndex += len;
					dat.unsure = true;
				...
				if(isFlagUnsigned(c)) {
					utils.copyTextIndex(dat.integerPastIndex, ti);
					dat.miniState =
						ExtendedIntTokenHelperData.STATE_FLAG_UNSIGNED;
				...
				if(isFlagLong(c)) {
					utils.copyTextIndex(dat.integerPastIndex, ti);
					dat.miniState =
						ExtendedIntTokenHelperData.STATE_FLAG_LONG;
					dat.flagLongCount = 1;
					dat.resultLength += len;
					dat.currentIndex += len;
					return;
			...
			if(state == ExtendedIntTokenHelperData.STATE_FLAG_UNSIGNED) {
				if(dat.flagLongCount == 0)
				if(isFlagLong(c)) {
					dat.miniState = ExtendedIntTokenHelperData.STATE_FLAG_LONG;
					dat.flagLongCount = 1;
					dat.resultLength += len;
					dat.currentIndex += len;
					return;
			...
			if(state == ExtendedIntTokenHelperData.STATE_FLAG_LONG) {
		...
		if(dat.unsure) {
			...
			if(state == ExtendedIntTokenHelperData.STATE_INTEGER_UNDERSCORE) {
				if(c == '_') {
				...
				if((dat.radix == 2) && utils.isBinaryChar(c)) {
					dat.miniState = ExtendedIntTokenHelperData.STATE_INTEGER;
			...
			if(state == ExtendedIntTokenHelperData.STATE_INITIAL_ZERO) {			
				if(c == 'h') {
					dat.miniState =
						ExtendedIntTokenHelperData.STATE_INTEGER_START;
					dat.radix = 16;
					//dat.resultLength += len;
					dat.unsureLength += len;
					dat.currentIndex += len;
			...
			if(state == ExtendedIntTokenHelperData.STATE_INTEGER_START) {
				if((dat.radix == 16) && utils.isHexChar(c)) {
