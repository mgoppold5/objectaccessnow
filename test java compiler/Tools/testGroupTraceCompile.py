...
def doName(dInfo, grpPath, nm):
	...
	clsFilePath = join(dInfo.clsDir, nm + clsExt)
	...
	if(os.path.isfile(trgPath)):
		...
		arcName = join(grpPath, nm)
		arcName = arcName.replace("/", "-")

		if(os.path.isfile(clsFilePath)):
			runWithCheck("rm"
				+ " " + QUOTE + clsFilePath + QUOTE)

		clsDir2 = join(dInfo.clsDir, grpPath)
		makeDirs2(clsDir2)

		makeDirs2(dInfo.arcDir)
		
		cmd = javaDir + "/bin/javac"
		if(not dInfo.clsPath is None):
			cmd += " -cp " + QUOTE + dInfo.clsPath + QUOTE
		cmd += " -d " + QUOTE + clsDir2 + QUOTE
		cmd += " " + QUOTE + trgPath + QUOTE
		runWithCheck(cmd)

		jarName = arcName + ".jar"
		cmd = javaDir + "/bin/jar cf"
		cmd += " " + join(dInfo.arcDir, jarName)
		cmd += " -C " + QUOTE + clsDir2 + QUOTE
		cmd += " ."
		runWithCheck(cmd)

		append = (not dInfo.clsPath is None)
		if(append):
			dInfo.clsPath += ":" + join(dInfo.arcDir, jarName)
		if(not append):
			dInfo.clsPath = join(dInfo.arcDir, jarName)
...
def doDir(dInfo, grpPath):
	...
	if(os.path.isfile(prefPath + listExt)):
		nList = loadStringListFromFile(prefPath + listExt)

		for nm3 in nList:
			if(len(nm3) == 0): continue

			doName(dInfo, grpPath, nm3)

			logicCount2 += dInfo.logicCount
			docCount2 += dInfo.docCount
