...
public class JsonTest {
	...
	public void initOnce() {
		...
		dat.jsonRead = new JsonBaseReader();
		dat.jsonRead.dat = new JsonBaseReaderData();
		dat.jsonRead.dat.init();
		dat.jsonRead.utils = dat.utils;
		//dat.jsonRead.charRead = dat.charRead;
		dat.jsonRead.tokChoose = dat.tokChoose;
		dat.jsonRead.initHelpers(dat.utils, dat.tokUtils);
	}
	...
	public boolean runTestFile() {
		...
		dat.jsonRead.move(ModuleMoveDirection.TO_NEXT);
		
		isError = false;
		
		while(true) {
			if(dat.jsonRead.dat.state == BaseModuleData.STATE_STUCK) {
				out.println(indentString()
					+ "STUCK with stuckState="
					+ dat.jsonRead.dat.stuckState);
				isError = true;
				break;
			}
			
			if(dat.jsonRead.dat.state == SimpleReaderData.STATE_HAVE_SYMBOL) {
				sym = dat.jsonRead.getSymbol();
			...
			moveResult = dat.jsonRead.move(ModuleMoveDirection.TO_NEXT);
		...
		checkModule(dat.jsonRead.dat, "JsonBaseReader");
