...
public class XmlTest {
	...
	public void initOnce() {
		...
		dat.xmlRead = new XmlBaseReader();
		dat.xmlRead.dat = new XmlBaseReaderData();
		dat.xmlRead.dat.init();
		dat.xmlRead.utils = dat.utils;
		dat.xmlRead.tokChoose = dat.tokChoose;
		dat.xmlRead.initHelpers(dat.utils);
	}
	...
	public void initTestFile(CommonInt32Array filePath) {
		...
		dat.linkUtils.initFileContext(dat.testFileNode, dat.fileDat);
		...
		dat.xmlRead.charRead = dat.charRead;
		dat.xmlRead.reset();
	...
	public boolean runTestFile() {
		...
		dat.xmlRead.move(ModuleMoveDirection.TO_NEXT);
		
		isError = false;
		
		while(true) {
			if(dat.xmlRead.dat.state == BaseModuleData.STATE_STUCK) {
				out.println(indentString()
					+ "STUCK with stuckState="
					+ dat.xmlRead.dat.stuckState);
				isError = true;
				break;
			}
			
			if(dat.xmlRead.dat.state == SimpleReaderData.STATE_HAVE_SYMBOL) {
				sym = dat.xmlRead.getSymbol();
				...
				if(sym.symbolType == SymbolTypes.TYPE_GRAM) {
					moveResult = dat.xmlRead.move(
						ModuleMoveDirection.TO_END);
			...
			moveResult = dat.xmlRead.move(ModuleMoveDirection.TO_NEXT);
		...
		checkModule(dat.xmlRead.dat, "XmlBaseReader");
	...
	private void dumpMove(int moveResult) {
		out.println(indentString()
			+ "ModuleMoveResult=" + moveResult);
		return;
	}
	...
	private String getSymbolString(Symbol sym) {
		...
		StringReader2Data strDat;

		strDat = (StringReader2Data) dat.strRead.getData();
	...
	private void dumpToken(Token tok) {
		...
		case Symbols.TOKEN_XML_TAG_FINISH:
			tokenName = "TOKEN_XML_TAG_FINISH";
			break;
		case Symbols.TOKEN_XML_TAG_FINISH_WITH_END_MARK:
			tokenName = "TOKEN_XML_TAG_FINISH_WITH_END_MARK";
			break;
		}
	...
	private void dumpGram(Gram grm) {
		...
		case Symbols.GRAM_XML_TAG_WITH_END_MARK:
			gramName = "GRAM_XML_TAG_WITH_END_MARK";
			break;
	...
	public void openLocalFileSystem(CommonInt32Array path, boolean writable) {
		...
		//fs.setFileNumbering(makeHandleSet(), 0);
	...
	public static void main(String[] args) {
		XmlTest o = new XmlTest();
		o.dat = new XmlTestData();
		o.out = System.out;
